...
@section('content')
    ...
    <div class="container ">
        <div class="card">
            <div class="card-body">
                ...
                <form id="create_education_post_form">
                    ...
                    <div class="row">
                        <div class="col-lg-6 col-12 ">
                            ...
                            @if ($subCategory->id == 28 || $subCategory->id == 31 || $subCategory->id == 32)
                            @else
                                <div class="col-12 mt-2 ">
                                    <label for="exampleInputEmail1">Type</label>
                                    <select class="browser-default custom-select  customSelect" name="educationType"
                                        id="educationType" aria-label="Default select example">
                                        <option value="" selected>Open this select menu</option>
                                        @foreach ($subCategoryTypes as $one)
                                            <option value="{{ $one->id }}">{{ $one->sct_name }}</option>
                                        @endforeach
                                    </select>
                                    <span class="text-danger error" id="educationType_error"></span>
                                </div>
                            @endif

                            @if ($subCategory->id == 29)
                                <div class="col-12 mt-2">
                                    <label for="exampleInputEmail1">Condition</label><br>
                                    <label class="radio-inline m-2">
                                        <input type="radio" name="Condition" value="New" class="m-1" checked>New
                                    </label>
                                    <label class="radio-inline m-2">
                                        <input type="radio" class="m-1" value="Used" name="Condition">Used
                                    </label>
                    ...
                    {{-- Features end  --}}
                    <button type="button" id="create_education_post" class="btn btn-primary">Post Ad</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        //   create education ad start 
        $('#create_education_post').click(function() {

            $('.error').html(''); //clear validation errors
            document.getElementById("create_education_post").disabled = true; //disable button after click it
            var data = $('#create_education_post_form')[0];
            var dataAjax = new FormData(data);

            // ajax call start
            $.ajax({

                url: "{{ route('web.dashboard.education.create') }}",
                method: "POST",
                ...
                success: function(response) {
                    document.getElementById("create_education_post").disabled =
                ...
                error: function(error) {
                    ...
                    $('#educationType_error').html(error.responseJSON.errors.educationType);
